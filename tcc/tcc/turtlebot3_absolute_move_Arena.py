...
parser = argparse.ArgumentParser(description="Program that allows robot name to be used with absolute move")
parser.add_argument("name",type=str,help="Name of the robot e.g. TB3_1")
parser.add_argument("posx",type=float,help="X coordinate the robot will move to")
parser.add_argument("posy",type=float,help="Y coordinate the robot will move to")
parser.add_argument("rota",type=float,help="rotation (in degrees) the robot will move to")
parser.add_argument("posx_ret",type=float,help="X coordinate the robot will return to")
parser.add_argument("posy_ret",type=float,help="Y coordinate the robot will return to")
parser.add_argument("rota_ret",type=float,help="rotation (in degrees) the robot will return to")
...
class Turtlebot3AbsoluteMove(Node):
    ...
    def __init__(self):
        ...
        self.inputs = [args.posx,args.posy,args.rota,args.posx_ret,args.posy_ret,args.rota_ret]
        self.flag = 0
        ...
        self.cmd_vel_pub = self.create_publisher(CmdVelMsg, f'{args.name}/cmd_vel', qos)
    ...
    def get_key(self):
        ...
        self.goal_position.x = self.inputs[0+(self.flag*3)]
        self.goal_position.y = self.inputs[1+(self.flag*3)]
        self.goal_heading = self.inputs[2+(self.flag*3)]
        self.flag = (self.flag+1)%2 ##keeps self.flag between 0 e 1!
